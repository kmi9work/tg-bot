...
module SuiteCrm
  class Client
    ...
    def get_entries(module_name:, ids: [], select_fields: [])
      params = {
        'session' => @session_id,
        'module_name' => module_name,
        'ids' => ids,
      ...
      request = SuiteCrm::Request.new(@conn).call(method: 'get_entries', params: params)
      response = JSON.parse(request.body)
    end

    def get_available_modules(filter: 'all')
      params = {
        'session' => @session_id,
        'filter' => filter
      }

      request = SuiteCrm::Request.new(@conn).call(method: 'get_available_modules', params: params)
      response = JSON.parse(request.body)
    end
    private
